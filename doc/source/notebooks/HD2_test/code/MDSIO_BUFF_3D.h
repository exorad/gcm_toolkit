...
C           which should be enough for most applications. We increase
C           by a factor of 70 to allow loading of large opacity files for
C           pkg exorad

      INTEGER size3dBuf
#ifdef ALLOW_FIZHI
      PARAMETER ( size3dBuf = (Nr+NrPhys)*700 )
#else
      PARAMETER ( size3dBuf = 2*Nr*700 )
#endif
