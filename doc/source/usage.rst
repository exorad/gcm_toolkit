...
The functionalities of ``gcmt`` is likely to expand in the future, since ``gcmt`` is in active development.
However, ``gcmt`` can already be used to perform a variety of tasks.
These docs explain how ``gcmt`` can be used to load data and do plots with the data.
...
Plotting
--------
``gcmt`` includes some very simple (but yet powerful and highly customizable) plotting functions that can be used in two different ways.
...
.. code-block:: python
        ...
        from gcmt import GCMT
        import gcmt.gcm_plotting as gcmp
...
.. autofunction:: gcmt.gcm_plotting.isobaric_slice

.. autofunction:: gcmt.gcm_plotting.zonal_mean

.. autofunction:: gcmt.gcm_plotting.time_evol
