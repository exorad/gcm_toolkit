...
def m_add_horizontal_average(gcmt, var_key, var_key_out=None, area_key='area_c', tag=None):
    ...
    """
    # print information
    gcmt.wrt.write_status('STAT', 'Calculate horizontal average')
    gcmt.wrt.write_status('INFO', 'Tag: ' + tag)
    gcmt.wrt.write_status('INFO', 'Variable to be plotted: ' + var_key)
    gcmt.wrt.write_status('INFO', 'Output variable: ' + var_key_out)
    gcmt.wrt.write_status('INFO', 'Area of grid cells: ' + area_key)
